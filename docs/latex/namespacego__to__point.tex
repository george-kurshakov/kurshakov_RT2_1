...
\begin{DoxyVerb}Change goal reaching state.
...
\begin{DoxyVerb}Odometry callback function for getting current position and yaw.

Args:
    msg: the */odom* message\end{DoxyVerb}
...
\begin{DoxyVerb}Set all the velocities to 0.
\end{DoxyVerb}
...
\begin{DoxyVerb}Fix the final yaw.
...
\begin{DoxyVerb}Fix the initial yaw.
...
\begin{DoxyVerb}Go straight ahead reaching the desired position.
...
\begin{DoxyVerb}The main function initializing the node with all its publishers, subscribers and actions.
\end{DoxyVerb}
...
\begin{DoxyVerb}Keep the angle within the constraints (pi and -pi).
...
\begin{DoxyVerb}go_to_point action implementation. A simple state machine for reaching the goal.
